...
public class Map {
	...
	Map(String nomLevel) throws FileNotFoundException{
		...
		String fichierTiles;
		//On ouvre le fichier o� sont contenu les infos sur la map
		fichier=new Scanner(new File("../core/assets/"+nomLevel));
		//Premi�re ligne la taille multipli� par les tiles
		...
		//Boucles de cr�ation de la map
		for(int i=0;i<taille.y;i++){
			for(int j=0;j<taille.x;j++){
				//l'id du tile est trait�
	...
	public boolean win(Personnage perso){
		//Si le perso a pris tout les objectifs et est positionn� au bonne endroit alors retourne vrai

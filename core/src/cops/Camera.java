...
public class Camera {
	...
	Camera(Personnage perso){ // On centre la caméra sur un personnage saisi en argument.
		...
		cam = new OrthographicCamera(60,60);
		cam.position.set(sonPerso.getLocalisation(), 0);
		cam.update();
	}
	
	public void update(){
		cam.position.set(sonPerso.getLocalisation(),0); // On centre la caméra sur le perso
		cam.update();
	}
